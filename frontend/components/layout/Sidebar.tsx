...
export function Sidebar({ user, isOpen = false, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'flex h-screen w-60 flex-col border-r border-gray-200 bg-white',
          // Mobile: fixed overlay, hidden/shown via transform
          'fixed inset-y-0 left-0 z-30 transition-transform duration-200',
          isOpen ? 'translate-x-0' : '-translate-x-full',
          // Desktop: always visible, static in flex layout
                ...
                href === '/dashboard'
                  ...
                  ? pathname === href ||
                    (pathname.startsWith(href) &&
                      pathname !== '/dashboard/leads/queue')
                  : pathname.startsWith(href);
              return (
                <li key={href}>
                  <Link
